	...
	implements ICFSecClusterTable
{
	...
	public CFSecBuffCluster ensureRec(ICFSecCluster rec) {
		return (((CFSecBuffClusterFactoryService)(schema.getCFSecBuffFactory().getFactoryCluster())).ensureRec(rec));
	}
		...
		ICFSecCluster iBuff )
	{
		...
		CFSecBuffClusterByUDescrIdxKey keyUDescrIdx = (CFSecBuffClusterByUDescrIdxKey)schema.getCFSecBuffFactory().getFactoryCluster().newByUDescrIdxKey();
		...
		if (Buff == null) {
			...
			if (classCode == ICFSecCluster.CLASS_CODE) {
				CFSecBuffCluster retbuff = ((CFSecBuffCluster)(schema.getCFSecBuffFactory().getFactoryCluster().newRec()));
				retbuff.set(Buff);
				return( retbuff );
			}
			else {
		...
		ICFSecCluster iBuff )
	{
		...
		CFSecBuffClusterByUDomNameIdxKey existingKeyUDomNameIdx = (CFSecBuffClusterByUDomNameIdxKey)schema.getCFSecBuffFactory().getFactoryCluster().newByUDomNameIdxKey();
		existingKeyUDomNameIdx.setRequiredFullDomName( existing.getRequiredFullDomName() );

		CFSecBuffClusterByUDomNameIdxKey newKeyUDomNameIdx = (CFSecBuffClusterByUDomNameIdxKey)schema.getCFSecBuffFactory().getFactoryCluster().newByUDomNameIdxKey();
		newKeyUDomNameIdx.setRequiredFullDomName( Buff.getRequiredFullDomName() );

		CFSecBuffClusterByUDescrIdxKey existingKeyUDescrIdx = (CFSecBuffClusterByUDescrIdxKey)schema.getCFSecBuffFactory().getFactoryCluster().newByUDescrIdxKey();
		existingKeyUDescrIdx.setRequiredDescription( existing.getRequiredDescription() );

		CFSecBuffClusterByUDescrIdxKey newKeyUDescrIdx = (CFSecBuffClusterByUDescrIdxKey)schema.getCFSecBuffFactory().getFactoryCluster().newByUDescrIdxKey();
